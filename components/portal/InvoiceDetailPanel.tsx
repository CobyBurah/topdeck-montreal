...
export function InvoiceDetailPanel({ invoice, onUpdate, onDelete, onBack }: InvoiceDetailPanelProps) {
  const locale = useLocale()
  const lead = invoice.lead || null
  ...
  useEffect(() => {
    ...
    setInvoiceForm({
      ...
      price: invoice.price != null ? String(invoice.price) : '',
    })
    setContactForm({
      full_name: invoice.customer?.full_name || lead?.full_name || '',
      email: invoice.customer?.email || lead?.email || '',
      phone: invoice.customer?.phone || lead?.phone || '',
      address: invoice.customer?.address || lead?.address || '',
    })
  }, [invoice, lead])
  ...
  const customerName = invoice.customer?.full_name || lead?.full_name || 'Unknown'
  const customerEmail = invoice.customer?.email || lead?.email || null
  const customerPhone = invoice.customer?.phone || lead?.phone || null
  const customerAddress = invoice.customer?.address || lead?.address || null
  const customerLanguage = invoice.customer?.language || lead?.language || 'en'
  const customerNotes = invoice.customer?.internal_notes || null
  ...
  const handleEditSave = async () => {
    ...
    await supabase
      .from('invoices')
      .update({
        invoice_id: invoiceForm.invoice_id || null,
        service: invoiceForm.service || null,
        price: invoiceForm.price ? parseFloat(invoiceForm.price) : null,
      })
      .eq('id', invoice.id)
  ...
  // Re-fetch invoice with full joins
  ...
  // Save fields on the invoices table
  const handleInvoiceFieldSave = async (fieldName: string, value: string | string[] | null) => {
    const supabase = createClient()
  ...
  const handleLeadFieldSave = async (fieldName: string, value: string | string[] | null) => {
    ...
    // Re-fetch the invoice to get updated lead data
    await refetchInvoice()
  }
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="px-6 py-3 border-b border-secondary-200 shrink-0">
        <div className="flex items-center gap-2">
          {invoice.invoice_id && (
            <a
              href={`https://app.squareup.com/dashboard/invoices/${invoice.invoice_id}`}
              target="_blank"
              rel="noopener noreferrer"
          ...
          {invoice.estimate_id && (
            <Link
              href={`/${locale}/employee-portal/estimates?estimateId=${invoice.estimate_id}`}
          ...
          {invoice.customer?.access_token && (
            <button
              onClick={() => {
                const link = `${window.location.origin}/auth/auto-login?token=${invoice.customer!.access_token}`
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {/* Invoice Details (from Square) */}
        <section>
          <h3 className="text-sm font-semibold text-secondary-500 uppercase tracking-wider mb-3">Invoice Details</h3>
          {isEditingInvoice ? (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-secondary-500 mb-1">Invoice ID</label>
                  <input
                    name="invoice_id"
            ...
            <div className="bg-secondary-50 rounded-xl p-4 space-y-3">
              ...
              {invoice.invoice_id && (
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-secondary-600">Invoice ID</span>
                  <span className="text-sm text-secondary-900 font-mono">{invoice.invoice_id}</span>
                </div>
              )}
              {invoice.service && (
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-secondary-600">Service</span>
                  <span className="text-sm text-secondary-900">{invoice.service}</span>
                </div>
              )}
              {invoice.invoice_id && (
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    <a
                      href={`https://app.squareup.com/pay-invoice/${invoice.invoice_id}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-sm text-primary-600 hover:text-primary-700 truncate max-w-[200px]"
                    >
                      pay-invoice/{invoice.invoice_id.slice(0, 8)}...
                    </a>
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(`https://app.squareup.com/pay-invoice/${invoice.invoice_id}`)
              ...
              {!formattedPrice && !invoice.invoice_id && !invoice.service && (
                <p className="text-sm text-secondary-400 text-center py-2">No invoice details yet</p>
              )}
            </div>
          )}
        </section>
