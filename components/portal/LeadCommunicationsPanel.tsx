...
interface LeadCommunicationsPanelProps {
  lead: Lead
...
export function LeadCommunicationsPanel({
  ...
  const { stage, context: stageContext, isLoading: isStageLoading } = useCustomerStage(lead.customer?.id || null)
  ...
  // Build a Customer object from the lead's customer data
  const customer: Customer | null = lead.customer
    ? {
        id: lead.customer.id,
        ...
        internal_notes: null,
        access_token: lead.customer.access_token || '',
        created_at: '',
        updated_at: '',
      }
    : null
