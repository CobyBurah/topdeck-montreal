...
import type { StainChoice, LeadCondition } from '@/types/lead'

const SQUARE_INVOICES_URL = 'https://app.squareup.com/dashboard/invoices/new'
...
export function CreateInvoiceModal({ estimate, isOpen, onClose }: CreateInvoiceModalProps) {
  const [copied, setCopied] = useState(false)

  const lead = estimate.lead || null
  const language = (estimate.customer?.language ?? lead?.language ?? 'en') as 'en' | 'fr'
  const stainChoices = Array.isArray(lead?.stain_choices) ? lead.stain_choices : []
  const clientPortalUrl = estimate.customer?.access_token && typeof window !== 'undefined'
    ? `${window.location.origin}/auth/auto-login?token=${estimate.customer.access_token}`
    : typeof window !== 'undefined'
      ? `${window.location.origin}/${language}/client-portal`
      : `/${language}/client-portal`
  const templateText = generateInvoiceTemplate(clientPortalUrl, language, stainChoices, lead?.condition ?? null)
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-secondary-200">
              <h2 className="text-lg font-semibold text-secondary-900">Create Invoice</h2>
              <button
                onClick={onClose}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-sm font-medium text-secondary-700">
                    Invoice Notes Template ({language === 'fr' ? 'French' : 'English'})
                  </p>
                  <button
                    onClick={handleCopy}
