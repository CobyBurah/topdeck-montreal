...
interface InvoiceCommunicationsPanelProps {
  invoice: Invoice
...
export function InvoiceCommunicationsPanel({
  invoice,
  timeline,
  isLoading,
  onBack,
}: InvoiceCommunicationsPanelProps) {
  ...
  const { stage, context: stageContext, isLoading: isStageLoading } = useCustomerStage(invoice.customer?.id || null)
  ...
  const customer: Customer | null = invoice.customer
    ? {
        ...
        internal_notes: null,
        access_token: invoice.customer.access_token || '',
        created_at: '',
        updated_at: '',
      }
    : null
