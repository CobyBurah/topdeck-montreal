...
export function PhotoGallery({
  ...
  }, [selectedIndex, viewMode, goToPrev, goToNext, closeLightbox])

  // Scroll thumbnail into view when selected index changes
  useEffect(() => {
    if (selectedIndex !== null && viewMode === 'single' && thumbnailContainerRef.current) {
          ...
          >
            ...
            {viewMode === 'single' ? (
              <>
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-4 pt-16 pb-8">
                  {/* Image container with nav buttons */}
                  <div className="relative flex-1 flex items-center justify-center w-full max-h-[calc(100vh-300px)]">
                    {/* Previous button */}
                    {photos.length > 1 && (
                      <button
                        onClick={(e) => {
